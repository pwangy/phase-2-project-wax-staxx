...
const Library = ({ showStaxx, albums, searchQuery, sortSelected }) => {
    const filterAndSortedAlbums = (albums, searchQuery, sortSelected, filterPageCondition) => {
        ...
        .map(a => <Card key={a.id} {...a} showStaxx={filterPageCondition(a) ? a.inCollection : false} />)
    }
    const renderLibrary = filterAndSortedAlbums(albums, searchQuery, sortSelected, () => true )
    const renderStaxx = filterAndSortedAlbums(albums, searchQuery, sortSelected, album => album.inCollection === true)
